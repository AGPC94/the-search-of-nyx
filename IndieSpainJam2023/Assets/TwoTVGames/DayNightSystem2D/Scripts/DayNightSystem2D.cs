...
public class DayNightSystem2D : MonoBehaviour
{
    ...
    [Tooltip("Midnight color, you can adjust based on best color for this cycle")]
    public Color earlyMorning; // Eg: 00:00 at 06:00
    ...
    void Start() 
    {
     ...
     void Update()
     {
        ...
        if(dayCycle > DayCycles.EarlyMorning)
            dayCycle = 0;
        ...
        if(dayCycle == DayCycles.Midnight)
            globalLight.color = Color.Lerp(midnight, earlyMorning, percent);

        // EarlyMorning state
        if (dayCycle == DayCycles.EarlyMorning)
            globalLight.color = Color.Lerp(earlyMorning, day, percent);
    }

     void ControlLightMaps(bool status)
     {
        // loop in light array of objects to enable/disable
        if (mapLights.Length > 0 || mapLights == null)
        {
    ...
    public bool IsDay()
    {
        return dayCycle >= DayCycles.Sunrise && dayCycle < DayCycles.Night;
    }

    public bool IsNight()
    {
        return dayCycle >= DayCycles.Night;
    }
}
